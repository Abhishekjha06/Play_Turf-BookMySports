...
const Bookings = () => {
  ...
  return (
    <MobileShell>
      <AppHeader />
      <h1 className="px-4 mt-4 font-display font-extrabold text-2xl">My Bookings</h1>
      ...
      <div className="px-4 mt-4 flex gap-2 bg-panel-2 rounded-full p-1 border border-white/5">
        {(["all", "upcoming", "past"] as Tab[]).map((t) => (
          <button
            ...
            onClick={() => setTab(t)}
            className={cn(
              "flex-1 py-2 text-xs font-semibold rounded-full uppercase tracking-wider pressable",
