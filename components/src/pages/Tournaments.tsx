...
function Stat({ icon: Icon, label }: { icon: typeof Trophy; label: string }) {
  return (
    <div className="bg-panel-2 rounded-xl px-2 py-2 inline-flex items-center gap-1.5 border border-white/5">
      <Icon className="h-3.5 w-3.5 text-primary" /> <span className="text-soft truncate">{label}</span>
    </div>
  );
}
